...
void*  Input::ProcessMouseThread(void* arg)
{
	...
	if (fmouse != NULL)
	{
		while (((Input*)arg)->mQuit == false) // so as long as mQuit is FALSE, this will endlessly loop
		{
			...
			if(((Input*)arg)->TheMouse.PositionX > MouseScreenXlimit)((Input*)arg)->TheMouse.PositionX = MouseScreenXlimit;
			if (((Input*)arg)->TheMouse.PositionX < 0) ((Input*)arg)->TheMouse.PositionX = 0;
			
			((Input*)arg)->TheMouse.PositionY += (mousey / 1.0f);
			
			if (((Input*)arg)->TheMouse.PositionY >  MouseScreenYlimit ) ((Input*)arg)->TheMouse.PositionY =  MouseScreenYlimit ;
...
int Input::AreYouMyKeyboard()
{
	...
	std::string pattern = "event-mouse"; //<-change event-kbd to event-mouse if your BT keyboard is a "mouse"
	std::string file = "";

	DIR *dir;
	struct dirent *ent;

	printf("Checking for active keyboard\nPlease note, you may need to alter line marked in AreYouMyKeyboard() in Input.cpp if no keyboard response occurs\n");
	...
	if ((dir = opendir("/dev/input/by-path/")) != nullptr)
	{
		while ((ent = readdir(dir)) != nullptr) // lets scan theough the event files that are available
		{
			...
			{
				kbd = "/dev/input/by-path/" + file; // yes we seem to ...but be aware of the issue with wireless keyboards, not responding
